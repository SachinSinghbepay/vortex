  ...
  return (
    <div className="mb-6">
      <div
        className={cn(
          "rounded-xl border",
          hc ? `${hc.border} ${hc.bg}` : "border-white/6 bg-white/2"
        )}
      >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-white/35">
              {result ? (
                ...
                "AI analyses all your goals simultaneously"
              )}
            </p>
          </div>
        ...
        {open && result && (
          <div className="space-y-4 border-t border-white/6 px-5 pt-4 pb-5">
            <div className="space-y-2">
              {result.goalSummaries.map((g) => (
                <div
                  key={g.id}
                  className="flex items-center gap-3 rounded-lg border border-white/6 bg-white/2 px-3 py-2.5"
                >
                  <div
                    className={cn(
                      "h-2 w-2 shrink-0 rounded-full",
                      statusDot[g.status] ?? "bg-white/20"
                    )}
                  />
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {result.priorityOrder.map((title, i) => (
                    <span
                      key={title}
                      className="flex items-center gap-1.5 rounded-full border border-white/8 bg-white/4 px-2.5 py-1 text-xs text-white/60"
                    >
            ...
            <div className="rounded-lg border border-violet-500/20 bg-violet-500/5 px-4 py-3">
              <p className="mb-1 text-[10px] font-medium tracking-widest text-violet-400/70 uppercase">
                Recommendation
              </p>
            ...
            <div key={actIdx}>
              <div className="mb-2 flex items-center justify-between">
                ...
                <span className="text-xs text-white/25">
                  {Math.round((activityWeeklyMins(act) / 60) * 10) / 10}h/week
                </span>
              </div>
                    ...
                    {editable ? (
                      <input
                        ...
                        onChange={(e) => {
                          const v = Math.max(
                            0,
                            Math.min(480, parseInt(e.target.value) || 0)
                          )
                      ...
                      <div
                        className={cn(
                          "w-full rounded-md py-1.5 text-center text-[11px] font-medium",
                          act[d] > 0
      ...
      {hasHabits && (
        <div className={cn("flex flex-wrap gap-2", hasActivities && "mt-4")}>
          {entry.habits.map((h, i) => (
            <div
              key={i}
              className="flex items-center gap-1.5 rounded-lg border border-white/6 bg-white/3 px-3 py-1.5"
            >
...
function ScheduleSection({
  ...
  const [questions, setQuestions] = useState<
    Array<{ goalId: string; question: string }>
  >([])
  ...
  const submitAnswer = async () => {
    ...
    if (newAnswers.length >= questions.length) {
      ...
      try {
        ...
        const byGoal = questions.reduce<
          Record<string, { questions: string[]; answers: string[] }>
        >((acc, q, i) => {
        ...
        const qa = Object.entries(byGoal).map(([goalId, v]) => ({
          goalId,
        ...
        if (res.ok) {
          setExtracted((await res.json()).schedule ?? {})
  ...
  const runOptimize = async () => {
    setLoading(true)
    try {
      const res = await fetch("/api/ai/schedule-optimize", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        ...
        <div className="flex items-center gap-4">
          <div className="flex-1">
            ...
            <p className="mt-0.5 text-xs text-white/35">
              AI asks the right questions for each goal — workouts, meals, study
              time, sleep — to build a complete picture
            </p>
          </div>
          <button
            onClick={startSetup}
      ...
      <div className="mb-6">
        <div className="mb-3 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-white/35">
              Workouts, study, meals, sleep — per goal
            </p>
          </div>
          <div className="flex gap-2">
            ...
            <button
              onClick={() => {
                setEditDraft(JSON.parse(JSON.stringify(schedule)))
                setStatus("editing")
              }}
        ...
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-1 rounded-lg border border-white/8 px-3 py-1.5 text-xs text-white/40 transition hover:text-white/70"
            >
              <X className="h-3 w-3" /> Cancel
            </button>
            <button
              onClick={() => saveSchedule(editDraft)}
              disabled={saving}
  ...
  // ── AI Optimize result ────────────────────────────────────────────────────
  if (status === "optimizing" && optimizeResult)
    return (
      <div className="mb-6 rounded-xl border border-violet-500/20 bg-violet-500/5 p-5">
        ...
        <div className="space-y-3">
          {Object.entries(optimizeResult.suggestedSchedule).map(
            ([id, entry]) => (
              <GoalCard key={id} goalId={id} entry={entry} />
            )
          )}
        ...
        <div className="mt-4 flex gap-2">
          <button
            onClick={() => saveSchedule(optimizeResult.suggestedSchedule)}
            disabled={saving}
          ...
          <button
            onClick={() => setStatus("view")}
            className="rounded-lg border border-white/8 px-4 py-2 text-xs text-white/40 transition hover:text-white/70"
          >
            Keep current
          </button>
        </div>
  ...
  if (status === "confirming" && extracted)
    return (
      <div className="mb-6 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-5">
        ...
        <button
          onClick={() => saveSchedule(extracted)}
          disabled={saving}
    ...
    <div className="mb-6 rounded-xl border border-white/6 bg-white/2 p-5">
      ...
      <div className="my-3 h-0.5 w-full rounded-full bg-white/6">
        <div
          ...
          style={{
            width: `${Math.round((currentQ / questions.length) * 100)}%`,
          }}
        />
      </div>
      {loading ? (
    ...
    >
      ...
      <div className="mb-8">
        ...
        <p className="mt-0.5 text-sm text-white/40">
          How your work maps to your goals
        </p>
      </div>
      ...
      <div className="mb-8 flex flex-wrap gap-3">
        {kpis.map((kpi) => (
          <div
            key={kpi.label}
            className="min-w-[180px] flex-1 rounded-xl border border-white/6 bg-white/2 p-4"
          >
            ...
            {kpi.sub && <p className="mt-1 text-xs text-white/30">{kpi.sub}</p>}
          </div>
        ))}
      </div>
      ...
      <div className="mb-8 rounded-xl border border-white/6 bg-white/2 p-5">
        <div className="mb-3 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-white/35">
              Based on stress levels and overdue tasks
            </p>
          </div>
          <span className={cn("text-sm font-semibold", bc.color)}>
            {bc.label}
          </span>
        </div>
        <div
          className={cn("h-2 w-full overflow-hidden rounded-full", bc.track)}
        >
          <div
                ...
                <div className="mb-1.5 flex items-center justify-between">
                  <div className="flex min-w-0 items-center gap-2">
                    <div
                      className={cn(
                        "h-2 w-2 shrink-0 rounded-full",
                        typeColor[goal.type] ?? "bg-white/30"
                      )}
                    />
                  ...
                  <span className="ml-3 shrink-0 text-xs font-medium text-white/50">
                    {goal.progress}%
                  </span>
                </div>
        ...
        weeklyFocusMinutes === 0 && (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="mt-1 text-xs text-white/20">
              Complete tasks, set goals, and log focus sessions to see your
              analytics
            </p>
          </div>
        )}
